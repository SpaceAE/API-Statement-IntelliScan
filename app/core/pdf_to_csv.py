...
_DESC_NOTE_HINT_RE = re.compile(
	r'\b(?:desc|description|รายละเอียด|note|บันทึก|บันทึกช่วยจำ|บันทึกช่วยจํา)\b', re.IGNORECASE
)
...
def _standardize_df(df: pd.DataFrame) -> pd.DataFrame:
	...
	df = df.copy()
	try:
		# pandas 2.2+ แนะนำ .map กับ DataFrame
		df = df.map(_cell_norm)  # type: ignore[attr-defined]
	except AttributeError:
		# เผื่อเครื่องบางเครื่องยังไม่มี DataFrame.map
	...
	before = len(df)
	df = df[~desc_series.str.contains('balance brought forward', case=False)]
	df = df[~desc_series.str.contains('ยอดคง.*ยกมา', case=False)]
	...
	# ---- FIX: ซ่อมให้เหลือ debit/credit อย่างใดอย่างหนึ่ง และเติม balance ให้ครบ ----
...
def _detect_bank_brand(file_obj, password: Optional[str]) -> Optional[str]:
	...
	# 2) ตกลงมาใช้ pdfplumber (อ่านเฉพาะ 1-2 หน้าแรก)
	try:
		file_obj.seek(0)
		with pdfplumber.open(file_obj, password=password) as pdf:
